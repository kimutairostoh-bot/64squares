...
async function sbSelect(table, order, cols) {
  ...
  if (!res.ok) {
    ...
    console.error('[sb] GET ' + table + ':', res.status, t);
    return [];
  }
  return res.json();
}
...
async function sbDelete(table, id) {
  ...
  if (!res.ok) {
    const t = await res.text();
    console.error('[sb] DELETE ' + table + ':', res.status, t);
    return false;
  }
  return true;
}
...
// ============================================
// DEFAULT DATA  (seeds an empty database)
// ============================================
const defaultData = {
  articles: [
    {
      id: 1, tag: 'Strategy',
      title: 'The Art of the Endgame: Rook & Pawn Mastery',
      excerpt: 'Understanding why endgames are where true chess understanding is revealed — and how the world\'s best navigated them.',
      content: 'The endgame is where champions are made. While openings can be memorized and middlegames can be calculated, the endgame demands pure understanding — an intuitive grasp of piece coordination, king activity, and pawn promotion that separates true masters from the rest.\n\nRook endgames constitute the vast majority of practical endgame situations, and mastering them is perhaps the single greatest investment a club player can make. The Lucena position and the Philidor defense are not merely theoretical curiosities; they are the pillars upon which practical rook endgame technique is built.\n\nThe key insight is activity. In rook endgames, the rook must be active at all costs. A passive rook is almost always losing; an active rook can often hold or even win seemingly hopeless positions.\n\nPractice these positions repeatedly until the moves become second nature. The champions of the game did not merely study endgames — they absorbed them.',
      date: 'May 2026', read_time: '8 min', published: true, image: '',
    },
    {
      id: 2, tag: 'History',
      title: 'Fischer vs. Spassky: The Match That Stopped the World',
      excerpt: 'A look back at the 1972 World Championship in Reykjavik — a Cold War showdown played on 64 squares.',
      content: 'In the summer of 1972, the world held its breath — not over a military confrontation, but over a chess match. In Reykjavik, Iceland, an eccentric American prodigy named Robert James Fischer faced the calm Soviet champion Boris Spassky in what would become the most watched chess match in history.\n\nThe backdrop was unmistakable: the Cold War was at its height. The Soviet chess machine had dominated the World Championship for decades, and Fischer\'s challenge was seen in Washington and Moscow alike as something far greater than sport.\n\nFischer won the match 12.5–8.5, becoming the 11th World Chess Champion and the first American to hold the title. He never defended it. The chess world was never quite the same.',
      date: 'Apr 2026', read_time: '12 min', published: true, image: '',
    },
    {
      id: 3, tag: 'Opening Theory',
      title: 'The Sicilian Dragon: Fire on the Board',
      excerpt: 'One of chess\'s most double-edged openings — why the Dragon keeps burning generations of players.',
      content: 'Few opening systems in chess generate as much heat as the Sicilian Dragon. Named for the pawn structure that resembles the constellation Draco, the Dragon has seduced generations of attacking players with its promise of sharp, uncompromising chess.\n\nThe position arises after 1.e4 c5 2.Nf3 d6 3.d4 cxd4 4.Nxd4 Nf6 5.Nc3 g6, with Black fianchettoing the bishop on g7 — the fabled Dragon bishop, the soul of the entire system.\n\nThe Dragon demands deep theoretical preparation. A single misplaced move can be catastrophic for either side. Yet this is precisely the appeal: the Dragon rewards the player who has studied hardest and calculated most precisely.',
      date: 'Mar 2026', read_time: '6 min', published: true, image: '',
    },
  ],
  players: [
    {
      id: 1, title: 'GM', name: 'Garry Kasparov', country: 'Russia', rating: '2851',
      style: 'Universal',
      bio: 'Widely regarded as the greatest chess player of all time, Garry Kasparov dominated competitive chess for two decades. Born in Baku in 1963, he became World Champion at 22 — the youngest in history at the time — defeating the formidable Anatoly Karpov in a legendary five-match series.',
      career: 'Kasparov defeated Karpov in five legendary World Championship matches from 1984 to 1990. He held the world number one ranking for 225 months. In 1997, he famously lost a match to IBM\'s Deep Blue.',
      achievements: [
        { title: 'World Chess Champion', year: '1985–2000' },
        { title: 'Peak FIDE Rating 2851', year: '1999' },
        { title: 'World number 1 for 225 months', year: '1984–2005' },
      ],
      best_games: [
        { title: 'Kasparov vs. Topalov — The Immortal Game', event: 'Wijk aan Zee', year: '1999' },
      ],
      image: '',
    },
    {
      id: 2, title: 'GM', name: 'Magnus Carlsen', country: 'Norway', rating: '2882',
      style: 'Universal',
      bio: 'Magnus Carlsen is a Norwegian grandmaster and the highest-rated player in chess history, achieving a peak rating of 2882 in 2014. Born in 1990, he became a grandmaster at just 13 years of age.',
      career: 'Carlsen became World Chess Champion in 2013 by defeating Viswanathan Anand. He defended the title four times. In 2022 he declined to defend against Nepomniachtchi.',
      achievements: [
        { title: 'FIDE World Chess Champion', year: '2013–2023' },
        { title: 'Peak FIDE Rating 2882', year: '2014' },
      ],
      best_games: [
        { title: 'Carlsen vs. Karjakin — Game 10', event: 'World Championship Match', year: '2016' },
      ],
      image: '',
    },
    {
      id: 3, title: 'GM', name: 'Bobby Fischer', country: 'USA', rating: '2785',
      style: 'Tactical',
      bio: 'Robert James Fischer, the 11th World Chess Champion. Born in 1943, became a grandmaster at 15 — a world record at the time. His intensity, genius, and turbulent personality made him a cultural phenomenon.',
      career: 'Fischer annihilated Taimanov and Larsen 6–0 in the 1971 Candidates before defeating Petrosian. His 1972 match against Spassky became a global media sensation.',
      achievements: [
  ...
  games: [
    {
      id: 1, title: 'The Opera Game',
      white: 'Paul Morphy', black: 'Duke of Brunswick & Count Isouard',
      white_name: 'Paul Morphy', black_name: 'Duke of Brunswick & Count Isouard',
      white_title: '', black_title: '',
      year: '1858', event: 'Paris Opera', result: '1-0',
      description: 'Played during a performance of Norma at the Paris Opera House, this game is the quintessential illustration of rapid development. Morphy declined material repeatedly, culminating in a breathtaking queen sacrifice on move 16.',
    ...
    {
      id: 2, title: 'The Immortal Game',
      white: 'Adolf Anderssen', black: 'Lionel Kieseritzky',
      white_name: 'Adolf Anderssen', black_name: 'Lionel Kieseritzky',
      white_title: '', black_title: '',
      year: '1851', event: 'London', result: '1-0',
      description: 'Anderssen sacrificed both rooks, his bishop, and finally his queen — then delivered checkmate with three minor pieces. Perhaps the most celebrated attacking game in chess history.',
    ...
    {
      id: 3, title: 'Game of the Century',
      white: 'Donald Byrne', black: 'Robert J. Fischer',
      white_name: 'Donald Byrne', black_name: 'Robert J. Fischer',
      white_title: '', black_title: '',
      year: '1956', event: 'Rosenwald Trophy', result: '0-1',
      description: 'A 13-year-old Bobby Fischer sacrificed his queen on move 17, launching a forcing sequence of extraordinary depth. Hans Kmoch declared it \'The Game of the Century\' — the name has stuck for 70 years.',
      pgn: '1.Nf3 Nf6 2.c4 g6 3.Nc3 Bg7 4.d4 O-O 5.Bf4 d5 6.Qb3 dxc4 7.Qxc4 c6 8.e4 Nbd7 9.Rd1 Nb6 10.Qc5 Bg4 11.Bg5 Na4 12.Qa3 Nxc3 13.bxc3 Nxe4 14.Bxe7 Qb6 15.Bc4 Nxc3 16.Bc5 Rfe8+ 17.Kf1 Be6 18.Bxb6 Bxc4+ 19.Kg1 Ne2+ 20.Kf1 Nxd4+ 21.Kg1 Ne2+ 22.Kf1 Nc3+ 23.Kg1 axb6 24.Qb4 Ra4 25.Qxb6 Nxd1 26.h3 Rxa2 27.Kh2 Nxf2 28.Re1 Rxe1 29.Qd8+ Bf8 30.Nxe1 Bd5 31.Nf3 Ne4 32.Qb8 b5 33.h4 h5 34.Ne5 Kg7 35.Kg1 Bc5+ 36.Kf1 Ng3+ 37.Ke1 Bb4+ 38.Kd1 Bb3+ 39.Kc1 Ne2+ 40.Kb1 Nc3+ 41.Kc1 Rc2#',
    },
  ],
  pdfs: [
    {
      id: 1, title: 'My System', author: 'Nimzowitsch', tag: 'Strategy',
      description: 'The foundational text of modern positional chess strategy.',
      content: 'Nimzowitsch\'s My System, published in 1925, is arguably the most influential chess book ever written. It systematized concepts like the blockade, prophylaxis, and overprotection into a coherent framework.\n\nNimzowitsch\'s prose is colorful; he anthropomorphizes pawns and pieces, describing the passed pawn\'s \'lust to expand.\' Whether this makes the book more or less accessible depends on the reader, but it is rarely dull.\n\nMy System remains essential reading for any player seeking to move beyond tactical calculation into strategic understanding.',
      size: '2.4 MB', url: '', file_data: '', file_name: '', cover_image: '',
    },
    {
      id: 2, title: 'Chess Fundamentals', author: 'Capablanca', tag: 'Endgames',
      description: 'The World Champion\'s essential guide covering endings, middle games, and openings.',
      content: 'Capablanca\'s Chess Fundamentals, published in 1921, is lean and direct. Capablanca believed chess knowledge should be built from the endgame backward — understand the endings first, and the rest of the game makes more sense.\n\nThe book covers king and pawn endgames, rook endgames, basic tactical motifs, and key opening principles, all illustrated with Capablanca\'s own games.\n\nFor the improving player, Chess Fundamentals remains one of the most efficient paths to genuine chess understanding.',
      size: '1.8 MB', url: '', file_data: '', file_name: '', cover_image: '',
    },
  ],
};
...
function detectOpening(pgn) {
  ...
  // Openings are already sorted longest-first above
...
function fenToBoard(fen) {
  return fen.split(' ')[0].split('/').map(function (row) {
    ...
    for (const ch of row) {
      if (isNaN(ch)) {
        r.push((ch === ch.toUpperCase() ? 'w' : 'b') + ch.toUpperCase());
      } else {
...
function parsePGN(pgn) {
  const moves = pgn
    ...
    .split(' ')
    .filter(function (m) {
      return m && !m.match(/^\d/) && !['1-0', '0-1', '1/2-1/2'].includes(m);
    });
  const states = [];
...
function applyMove(board, san, color) {
  const b = board.map(function (r) { return [...r]; });
  try {
    if (san === 'O-O' || san === '0-0') {
      const row = color === 'w' ? 7 : 0;
      b[row][6] = color + 'K'; b[row][4] = null;
      b[row][5] = color + 'R'; b[row][7] = null;
      return b;
    }
    if (san === 'O-O-O' || san === '0-0-0') {
      const row = color === 'w' ? 7 : 0;
      b[row][2] = color + 'K'; b[row][4] = null;
      b[row][3] = color + 'R'; b[row][0] = null;
      return b;
    }
    ...
    for (let r = 0; r < 8; r++) {
      for (let c = 0; c < 8; c++) {
        ...
        if (hint) {
          if (hint.length === 2) {
            if (r !== 8 - parseInt(hint[1]) || c !== hint.charCodeAt(0) - 97) continue;
          } else if (isNaN(hint)) {
            if (c !== hint.charCodeAt(0) - 97) continue;
          } else {
            if (r !== 8 - parseInt(hint)) continue;
          }
        }
...
function getLegalMoves(board, r, c, color) {
  ...
  if (type === 'P') {
    ...
    for (const dc of [-1, 1]) {
      if (inBounds(r + dir, c + dc) && board[r + dir][c + dc] && board[r + dir][c + dc][0] === enemy)
...
function getHomePlayers() {
  const w = siteData.players[0];
  const b = siteData.players[1];
  const whiteName = w ? ((w.title ? w.title + ' ' : '') + w.name) : 'White';
  const blackName = b ? ((b.title ? b.title + ' ' : '') + b.name) : 'Black';
...
function buildHomeBoard() {
  ...
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      ...
      if (homeMoves.some(function (m) { return m[0] === r && m[1] === c; })) {
        sq.classList.add('isq-legal');
        if (homeBoard[r][c]) sq.classList.add('isq-capture');
      }

      sq.addEventListener('click', (function (rr, cc) {
        return function () { homeClick(rr, cc); };
      })(r, c));
      el.appendChild(sq);
    }
  }
  ...
  if (ti) ti.textContent = homeTurn === 'w'
    ? '\u2B1C ' + whiteName + ' to move'
    : '\u2B1B ' + blackName + ' to move';
...
// ============================================
// Fix: use string keys throughout so numeric game IDs don't cause mismatches
...
// vid must always be a string
function initGameViewer(vid, pgn) {
  const key    = String(vid);
  const states = pgn ? parsePGN(pgn) : [fenToBoard(START_FEN)];
  gameViewerStates[key] = { states, idx: 0, flipped: false, playing: false, timer: null };
  buildGameBoard('board-' + key, states[0], false);
  updateGameNav(key);
  renderClickablePgn(key, pgn);
}

function renderClickablePgn(vid, pgn) {
  const key = String(vid);
  const el  = document.getElementById('pgn-' + key);
  if (!el || !pgn) return;
  const tokens = pgn.replace(/\s+/g, ' ').trim().split(' ');
  let html = '', mi = 0;
  for (const tok of tokens) {
    if (tok.match(/^\d+\./)) {
      html += '<span class="pgn-movenum">' + tok + '</span> ';
    } else if (['1-0', '0-1', '1/2-1/2'].includes(tok)) {
      html += '<span class="pgn-result">' + tok + '</span>';
    } else {
      mi++;
      html += '<span class="pgn-move" data-idx="' + mi + '" onclick="pgnMoveClick(\'' + key + '\',' + mi + ')">' + tok + '</span> ';
    }
  }
  el.innerHTML = html;
}
...
function toggleAutoPlay(vid) {
  const key = String(vid);
  const vs  = gameViewerStates[key];
  if (!vs) return;
  if (vs.playing) stopAutoPlay(key); else startAutoPlay(key);
}
...
// Keyboard navigation for the active viewer
...
// ============================================
// WHAT TO READ NEXT
// Fix: pick items once and cache so they don't reshuffle on re-render
// ============================================
function buildWhatNext(currentType, currentId) {
  const items = [];
  const arts = siteData.articles.filter(function (a) {
    return a.published !== false && (currentType !== 'article' || a.id !== currentId);
  });
  const plrs = siteData.players.filter(function (p) {
    return currentType !== 'player' || p.id !== currentId;
  });
  const gms  = siteData.games.filter(function (g) {
    return currentType !== 'game' || g.id !== currentId;
  });

  // Pick deterministically by rotating through available items
  const pick = function (arr) { return arr.length ? arr[currentId % arr.length] : null; };

  const a = pick(arts);
  const p = pick(plrs);
  const g = pick(gms);

  if (a) items.push({ type: 'article', id: a.id, label: a.tag || 'Article', title: a.title,
    sub: (a.date || '') + (a.read_time ? ' &middot; ' + a.read_time + ' read' : '') });
  if (p) items.push({ type: 'player', id: p.id, label: 'Player',
    title: (p.title ? p.title + ' ' : '') + p.name, sub: p.country || '' });
  if (g) items.push({ type: 'game', id: g.id, label: 'Game', title: g.title,
    sub: g.white + ' vs ' + g.black });
  ...
  return '<div class="whatnext-strip">'
    ...
    + items.map(function (it) {
        const oc = it.type === 'article' ? 'openArticle(' + it.id + ')'
          : it.type === 'player' ? 'openPlayer(' + it.id + ')'
          : 'openGame(' + it.id + ')';
        return '<div class="whatnext-card" onclick="' + oc + '">'
...
function buildLatestStrip() {
  const el = document.getElementById('latestStrip');
  if (!el) return;
  const all = [];
  siteData.articles.filter(function (a) { return a.published !== false; }).forEach(function (a) {
    all.push({ type: 'article', id: a.id, label: a.tag || 'Article', title: a.title, ts: a.id });
  });
  siteData.players.forEach(function (p) {
    all.push({ type: 'player', id: p.id, label: 'Player',
      title: (p.title ? p.title + ' ' : '') + p.name, ts: p.id });
  });
  siteData.games.forEach(function (g) {
    all.push({ type: 'game', id: g.id, label: 'Game', title: g.title, ts: g.id });
  });
  ...
  el.innerHTML = '<div class="latest-inner">'
    ...
    + recent.map(function (it) {
        const oc = it.type === 'article' ? 'openArticle(' + it.id + ')'
          : it.type === 'player' ? 'openPlayer(' + it.id + ')'
          : 'openGame(' + it.id + ')';
        return '<div class="latest-card" onclick="' + oc + '">'
...
function runSearch() {
  ...
  if (!q) {
    res.innerHTML = '<p class="search-empty">Start typing to search articles, players, games and books.</p>';
    return;
  }
  const hits = [];
  siteData.articles.filter(function (a) { return a.published !== false; }).forEach(function (a) {
    if ((a.title + ' ' + (a.tag || '') + ' ' + (a.excerpt || '')).toLowerCase().includes(q))
      hits.push({ label: a.tag || 'Article', title: a.title,
        sub: (a.date || '') + (a.read_time ? ' &middot; ' + a.read_time + ' read' : ''),
        onclick: 'openArticle(' + a.id + ')' });
  });
  siteData.players.forEach(function (p) {
    if ((p.name + ' ' + (p.country || '') + ' ' + (p.bio || '')).toLowerCase().includes(q))
      hits.push({ label: p.title || 'Player', title: p.name,
        sub: (p.country || '') + (p.rating ? ' &middot; ' + p.rating : ''),
        onclick: 'openPlayer(' + p.id + ')' });
  });
  siteData.games.forEach(function (g) {
    if ((g.title + ' ' + g.white + ' ' + g.black + ' ' + (g.event || '')).toLowerCase().includes(q))
      hits.push({ label: 'Game ' + g.year, title: g.title,
        sub: g.white + ' vs ' + g.black,
        onclick: 'openGame(' + g.id + ')' });
  });
  siteData.pdfs.forEach(function (p) {
    if ((p.title + ' ' + p.author + ' ' + (p.description || '')).toLowerCase().includes(q))
      hits.push({ label: p.tag || 'PDF', title: p.title, sub: 'by ' + p.author,
        onclick: 'openPdf(' + p.id + ')' });
  });

  if (!hits.length) {
    res.innerHTML = '<p class="search-empty">No results for &ldquo;' + q + '&rdquo;</p>';
    return;
  }
...
function openArticle(id) {
  const a = siteData.articles.find(function (x) { return x.id === id; });
  if (!a) return;
  ...
  const heroImg = a.image
    ? '<img class="article-detail-hero-img" src="' + a.image + '" alt="' + escHtml(a.title) + '"/>'
    : '';
  const related = siteData.articles.filter(function (x) {
    return x.published !== false && x.id !== id && x.tag === a.tag;
  }).slice(0, 2);
  const relatedHTML = related.length
    ? '<div class="related-strip"><div class="related-title">More in ' + escHtml(a.tag) + '</div>'
      + '<div class="related-cards">'
      + related.map(function (r) {
          return '<div class="related-card" onclick="openArticle(' + r.id + ')">'
            + '<div class="related-tag">' + escHtml(r.tag) + '</div>'
            + '<div class="related-card-title">' + escHtml(r.title) + '</div>'
            + '<div class="related-card-meta">' + (r.read_time || '') + '</div>'
            + '</div>';
        }).join('')
      + '</div></div>'
    : '';
  ...
  showDetailPage(
    ...
    + '<div class="article-detail">'
      + heroImg
      + '<div class="article-detail-tag">' + escHtml(a.tag || 'General') + '</div>'
      + '<h1 class="article-detail-title">' + escHtml(a.title) + '</h1>'
      + '<div class="article-detail-meta">'
        + escHtml(a.date || '') + ' &nbsp;&middot;&nbsp; ' + escHtml(a.read_time || '') + ' read'
      + '</div>'
...
function openPlayer(id) {
  ...
  const avatarHTML = p.image
    ? '<img class="player-detail-photo" src="' + p.image + '" alt="' + escHtml(p.name) + '"/>'
    : '<div class="player-detail-avatar">' + escHtml(p.name[0]) + '</div>';

  const achievements = Array.isArray(p.achievements) ? p.achievements : [];
  const bestGames    = Array.isArray(p.best_games)   ? p.best_games   : [];

  const achievementsHTML = achievements.map(function (ach) {
    return '<tr><td>' + escHtml(ach.title) + '</td><td>' + escHtml(ach.year) + '</td></tr>';
  }).join('') || '<tr><td colspan="2" style="color:var(--mid);font-size:.8rem;">No achievements listed.</td></tr>';

  const bestGamesHTML = bestGames.map(function (g) {
    return '<div class="bestgame-row">'
      + '<h4>' + escHtml(g.title) + '</h4>'
      + '<p>' + escHtml(g.event) + ' &middot; ' + escHtml(g.year) + '</p>'
      + '</div>';
  }).join('') || '<p style="color:var(--mid);font-size:.9rem;">No games listed.</p>';

  const styleBadge = p.style ? '<span class="style-badge">' + escHtml(p.style) + '</span>' : '';
  ...
  showDetailPage(
    ...
    + '<div class="player-detail">'
      + '<div class="player-detail-hero">'
        ...
        + '<div>'
          + '<div class="player-detail-name">'
            ...
            + escHtml(p.name)
          + '</div>'
          + '<div class="player-detail-country">' + escHtml((p.country || '').toUpperCase()) + '</div>'
          + '<div class="player-detail-rating-badge">Peak Rating: ' + escHtml(p.rating || 'N/A') + '</div>'
          + styleBadge
        + '</div>'
      + '</div>'
      ...
      + '</div>'
      + '<div class="pd-panel active" id="pd-bio"><p>' + escHtml(p.bio || 'No biography available.') + '</p></div>'
      + '<div class="pd-panel" id="pd-achievements"><table class="achievement-table"><tbody>' + achievementsHTML + '</tbody></table></div>'
      + '<div class="pd-panel" id="pd-career"><p>' + escHtml(p.career || 'No career summary available.') + '</p></div>'
...
function switchPdTab(btn, id) {
  ...
  document.querySelectorAll('.pd-panel').forEach(function (t) { t.classList.remove('active'); });
...
function openGame(id) {
  const g = siteData.games.find(function (x) { return x.id === id; });
  if (!g) return;
  trackView('game', id);

  // Use string key for viewer consistency
  const vid     = 'd' + id;
  const boardId = 'board-' + vid;
  const opening = detectOpening(g.pgn);
  const openingBadge = opening
    ? '<span class="opening-badge">\u265E ' + escHtml(opening) + '</span>'
    : '';
  ...
  showDetailPage(
    ...
    + '<div class="game-detail">'
      + '<div class="game-detail-header">'
        ...
        + '<h1 class="game-detail-title">' + escHtml(g.title) + '</h1>'
        + openingBadge
        + '<div class="game-detail-players">'
          + escHtml(g.white) + ' (White) vs ' + escHtml(g.black) + ' (Black)'
          + ' &nbsp;&middot;&nbsp; ' + escHtml(g.event || '') + ' ' + escHtml(g.year || '')
          + ' &nbsp;&middot;&nbsp; ' + escHtml(g.result || '')
        + '</div>'
      + '</div>'
      + '<div class="game-detail-body">'
        + '<div class="game-detail-left">'
          + '<p class="game-detail-desc">' + escHtml(g.description || 'A landmark game in chess history.') + '</p>'
          + (g.pgn
            ? '<div class="game-detail-pgn-label">PGN Notation</div>'
              + '<div class="game-detail-pgn-box"><div id="pgn-' + vid + '"></div></div>'
            : '')
        + '</div>'
        + '<div class="game-detail-right">'
          + '<div class="board-player-strip board-player-black">\u265A ' + escHtml(blackLbl) + '</div>'
          + '<div id="' + boardId + '" class="chess-board-viewer"></div>'
          + '<div class="board-player-strip board-player-white">\u2654 ' + escHtml(whiteLbl) + '</div>'
          + '<div class="gv-nav">'
            + '<button class="gv-btn" title="Start" onclick="gameJump(\'' + vid + '\',\'start\')">\u007C\u25C0</button>'
            + '<button class="gv-btn" id="prev-' + vid + '" onclick="gameStep(\'' + vid + '\',-1)">\u25C0</button>'
            + '<button class="gv-btn" id="play-' + vid + '" onclick="toggleAutoPlay(\'' + vid + '\')">\u25B6</button>'
            + '<span class="gv-movenav" id="movenav-' + vid + '">Move 0</span>'
            + '<button class="gv-btn" id="next-' + vid + '" onclick="gameStep(\'' + vid + '\',1)">\u25B6</button>'
            + '<button class="gv-btn" title="End" onclick="gameJump(\'' + vid + '\',\'end\')">\u25B6\u007C</button>'
            + '<button class="gv-btn" id="flip-' + vid + '" title="Flip" onclick="flipBoard(\'' + vid + '\')">&#8645;</button>'
          + '</div>'
        + '</div>'
      + '</div>'
...
async function openPdf(id) {
  ...
  const contentHTML = (p.content || '')
    ...
    .map(function (x) { return '<p>' + escHtml(x.trim()) + '</p>'; }).join('');

  let downloadBtn;
  if (p.file_data) {
    downloadBtn = '<a class="pdf-download-btn" href="' + p.file_data
      + '" download="' + escHtml(p.file_name || p.title + '.pdf') + '">\u2193 Download PDF</a>';
  } else if (p.url) {
    downloadBtn = '<a class="pdf-download-btn" href="' + escHtml(p.url)
      + '" target="_blank" rel="noopener">\u2193 Open PDF</a>';
  } else {
    downloadBtn = '<button class="pdf-download-btn" onclick="showToast(\'No file attached.\')">\u2193 Download PDF</button>';
  }

  let viewerHTML = '';
  const viewerSrc = p.file_data || p.url;
  if (viewerSrc) {
    viewerHTML = '<div style="margin-top:2rem;">'
      ...
      + '<iframe class="pdf-viewer-embed" src="' + viewerSrc + '" title="' + escHtml(p.title) + '"></iframe>'
      + '</div>';
  }

  const coverImg = p.cover_image
    ? '<img src="' + p.cover_image + '" alt="' + escHtml(p.title)
      + '" style="width:100%;max-height:280px;object-fit:cover;margin-bottom:2rem;border:1px solid var(--rule);">'
    : '';
  const tagBadge = p.tag
    ? '<span class="pdf-tag-badge">' + escHtml(p.tag) + '</span>'
    : '';
  ...
  showDetailPage(
    ...
    + '<div class="pdf-detail">'
      ...
      + '<div class="pdf-detail-header">'
        ...
        + '<div>'
          + '<h1 class="pdf-detail-title">' + escHtml(p.title) + '</h1>'
          + '<div class="pdf-detail-author">by ' + escHtml(p.author) + '</div>'
          + tagBadge
          + (p.size ? '<div class="pdf-detail-size">' + escHtml(p.size) + '</div>' : '')
          + (p.file_name
            ? '<div style="font-family:var(--mono);font-size:.6rem;color:var(--mid);margin-top:.3rem;">File: '
              + escHtml(p.file_name) + '</div>'
            : '')
        + '</div>'
      + '</div>'
      + (p.description ? '<p class="pdf-detail-desc">' + escHtml(p.description) + '</p>' : '')
      + downloadBtn
      + viewerHTML
      + (contentHTML
        ? '<div class="pdf-detail-content" style="margin-top:2.5rem;padding-top:2.5rem;border-top:1px solid var(--rule);">'
          + contentHTML + '</div>'
        : '')
...
function renderArticles() {
  ...
  grid.innerHTML = pub.map(function (a) {
    return '<div class="article-card fade-in" onclick="openArticle(' + a.id + ')">'
      + (a.image ? '<img class="article-card-img" src="' + a.image + '" alt="' + escHtml(a.title) + '"/>' : '')
...
function renderPlayers() {
  ...
  grid.innerHTML = siteData.players.map(function (p) {
    const avatarHTML = p.image
    ...
    return '<div class="player-card fade-in" onclick="openPlayer(' + p.id + ')">'
      ...
      + '<div class="player-card-body">'
        ...
        + (p.style
          ? '<div style="font-family:var(--mono);font-size:.6rem;color:var(--mid);letter-spacing:.1em;text-transform:uppercase;margin-top:.5rem;">'
            + escHtml(p.style) + '</div>'
          : '')
        + '<div style="margin-top:1rem;font-family:var(--mono);font-size:.62rem;color:var(--mid);letter-spacing:.08em;">'
          + 'Click to view full profile \u2192</div>'
      + '</div>'
      + '</div>';
  }).join('');
}
...
function renderGames() {
  ...
  list.innerHTML = siteData.games.map(function (g, i) {
    const opening = detectOpening(g.pgn);
    // Use string IDs for game viewer to avoid type mismatch
    const gid = String(g.id);
    const blackName = escHtml(g.black_name || g.black || 'Black');
    const whiteName = escHtml(g.white_name || g.white || 'White');
    return '<div class="game-entry fade-in">'
      + '<div class="game-row" onclick="toggleGameViewer(\'gv-' + gid + '\',' + gid + ')">'
        + '<div class="game-num">' + String(i + 1).padStart(2, '0') + '</div>'
        + '<div>'
          + '<div class="game-title">'
            + escHtml(g.title)
            + (opening ? ' <span class="opening-badge-sm">\u265E ' + escHtml(opening) + '</span>' : '')
          + '</div>'
          + '<div class="game-meta">'
            + escHtml(g.white || '') + ' vs ' + escHtml(g.black || '')
            + ' &nbsp;&middot;&nbsp; ' + escHtml(g.event || '')
            + ' &nbsp;&middot;&nbsp; ' + escHtml(g.result || '')
          + '</div>'
        + '</div>'
        + '<div class="game-right">'
          + '<div class="game-year">' + escHtml(g.year || '') + '</div>'
          + '<div class="game-expand-icon">\u25BE</div>'
        + '</div>'
      + '</div>'
      + '<div class="game-viewer" id="gv-' + gid + '" style="display:none;">'
        + '<div class="game-viewer-inner">'
          + '<div class="gv-board-wrap">'
            + '<div class="board-player-strip board-player-black">\u265A ' + blackName + '</div>'
            + '<div id="board-' + gid + '" class="chess-board-viewer"></div>'
            + '<div class="board-player-strip board-player-white">\u2654 ' + whiteName + '</div>'
          + '</div>'
          + '<div class="gv-controls">'
            + '<div class="gv-pgn" id="pgn-' + gid + '"></div>'
            + '<div class="gv-nav">'
              + '<button class="gv-btn" onclick="gameJump(\'' + gid + '\',\'start\')">\u007C\u25C0</button>'
              + '<button class="gv-btn" id="prev-' + gid + '" onclick="gameStep(\'' + gid + '\',-1)">\u25C0</button>'
              + '<button class="gv-btn" id="play-' + gid + '" onclick="toggleAutoPlay(\'' + gid + '\')">\u25B6</button>'
              + '<span class="gv-movenav" id="movenav-' + gid + '">Move 0</span>'
              + '<button class="gv-btn" id="next-' + gid + '" onclick="gameStep(\'' + gid + '\',1)">\u25B6</button>'
              + '<button class="gv-btn" onclick="gameJump(\'' + gid + '\',\'end\')">\u25B6\u007C</button>'
              + '<button class="gv-btn" id="flip-' + gid + '" onclick="flipBoard(\'' + gid + '\')">&#8645;</button>'
            + '</div>'
            + '<button class="gv-open-page" onclick="openGame(' + gid + ')">Open full game page \u2192</button>'
          + '</div>'
        + '</div>'
      + '</div>'
      + '</div>';
...
function toggleGameViewer(id, gameId) {
  ...
  if (!isOpen) {
    ...
    _activeViewerGameId = gid;
    const game = siteData.games.find(function (g) { return String(g.id) === gid; });
    if (game) {
...
function renderPDFs() {
  ...
  const cards = filtered.map(function (p) {
    ...
    return '<div class="pdf-card fade-in" onclick="openPdf(' + p.id + ')">'
      + (p.cover_image
        ? '<img class="pdf-card-cover" src="' + p.cover_image + '" alt="' + escHtml(p.title) + '"/>'
        : '')
      + '<div class="pdf-icon">PDF</div>'
      + (p.tag ? '<div class="pdf-card-tag">' + escHtml(p.tag) + '</div>' : '')
      + '<div class="pdf-title">' + escHtml(p.title) + '</div>'
      + '<div class="pdf-desc">'
        + escHtml(p.description || '')
        + '<br><br><em style="font-size:.78rem;">\u2014 ' + escHtml(p.author) + '</em>'
      + '</div>'
      + '<div class="pdf-size">'
        + (hasFile ? '\u2193 View / Download' : '\u2193 View')
        + ' &nbsp;&middot;&nbsp; ' + escHtml(p.size || '')
      + '</div>'
      + '</div>';
...
function handleImgUpload(inputId, previewId, dataId) {
  ...
  reader.onload = function (e) {
    ...
    const dropId = inputId.replace('-input', '-drop');
...
function renderAdminLists() {
  const pubA   = siteData.articles.filter(function (a) { return a.published !== false; });
  const draftA = siteData.articles.filter(function (a) { return a.published === false; });

  const al = document.getElementById('adminArticleList');
  if (al) al.innerHTML = pubA.map(function (a) {
    return '<div class="admin-item"><span>' + escHtml(a.title) + '</span>'
      + '<div class="admin-item-actions">'
        + '<button class="admin-edit-btn" onclick="editArticle(' + a.id + ')">Edit</button>'
        + '<button class="admin-del-btn" onclick="deleteItem(\'articles\',' + a.id + ')">\u2715</button>'
      + '</div></div>';
  }).join('') || '<p class="admin-empty">None yet.</p>';

  const dal = document.getElementById('adminArticleDraftList');
  if (dal) dal.innerHTML = draftA.map(function (a) {
    return '<div class="admin-item"><span>' + escHtml(a.title)
      + ' <span class="draft-badge">Draft</span></span>'
      + '<div class="admin-item-actions">'
        + '<button class="admin-edit-btn" onclick="editArticle(' + a.id + ')">Edit</button>'
        + '<button class="admin-del-btn" onclick="deleteItem(\'articles\',' + a.id + ')">\u2715</button>'
      + '</div></div>';
  }).join('') || '<p class="admin-empty">No drafts.</p>';

  const pl = document.getElementById('adminPlayerList');
  if (pl) pl.innerHTML = siteData.players.map(function (p) {
    return '<div class="admin-item"><span>'
      + escHtml((p.title ? p.title + ' ' : '') + p.name + (p.country ? ' — ' + p.country : ''))
      + '</span>'
      + '<div class="admin-item-actions">'
        + '<button class="admin-edit-btn" onclick="editPlayer(' + p.id + ')">Edit</button>'
        + '<button class="admin-del-btn" onclick="deleteItem(\'players\',' + p.id + ')">\u2715</button>'
      + '</div></div>';
  }).join('') || '<p class="admin-empty">No players yet.</p>';

  const gl = document.getElementById('adminGameList');
  if (gl) gl.innerHTML = siteData.games.map(function (g) {
    return '<div class="admin-item"><span>'
      + escHtml(g.title + ' — ' + g.white + ' vs ' + g.black + ' (' + g.year + ')')
      + '</span>'
      + '<div class="admin-item-actions">'
        + '<button class="admin-edit-btn" onclick="editGame(' + g.id + ')">Edit</button>'
        + '<button class="admin-del-btn" onclick="deleteItem(\'games\',' + g.id + ')">\u2715</button>'
      + '</div></div>';
  }).join('') || '<p class="admin-empty">No games yet.</p>';

  const dl = document.getElementById('adminPdfList');
  if (dl) dl.innerHTML = siteData.pdfs.map(function (p) {
    return '<div class="admin-item"><span>'
      + escHtml(p.title + ' — ' + p.author)
      + '</span>'
      + '<div class="admin-item-actions">'
        + '<button class="admin-edit-btn" onclick="editPdf(' + p.id + ')">Edit</button>'
        + '<button class="admin-del-btn" onclick="deleteItem(\'pdfs\',' + p.id + ')">\u2715</button>'
      + '</div></div>';
...
async function addArticle() {
  ...
  const content   = v('a-content');
  const excerpt   = v('a-excerpt');
  const date      = v('a-date');
  const rt        = v('a-readtime');
...
function editArticle(id) {
  ...
  if (a.image) {
    ...
    const drop = document.getElementById('a-img-drop');
    if (drop) drop.style.display = 'none';
  }
...
function previewArticle() {
  ...
  const bodyHTML = (content || excerpt || '')
    .split('\n').filter(function (p) { return p.trim(); })
    .map(function (p) { return '<p>' + p.trim() + '</p>'; }).join('');
  showDetailPage(
    '<div onclick="goHome();setTimeout(function(){openAdminPanel();},100)" class="detail-back">\u2190 Back to Editor</div>'
    + '<div class="article-detail">'
      + (image ? '<img class="article-detail-hero-img" src="' + image + '" alt="' + escHtml(title) + '"/>' : '')
      + '<div class="article-detail-tag">'
        + escHtml(tag || 'General')
        + ' &nbsp;<span style="background:#fff3cd;color:#856404;padding:.2rem .5rem;font-size:.6rem;">PREVIEW</span>'
      + '</div>'
      + '<h1 class="article-detail-title">' + escHtml(title || 'Untitled') + '</h1>'
      + '<div class="article-detail-meta">' + escHtml(date || nowDate()) + ' &nbsp;&middot;&nbsp; ' + escHtml(rt || '5 min') + ' read</div>'
...
async function addPlayer() {
  ...
  const name    = v('p-name');
  const country = v('p-country');
  const rating  = v('p-rating');
  const bio     = v('p-bio');
  const career  = v('p-career');
  ...
  const best_games = v('p-bestgames').split('\n').filter(function (l) { return l.trim(); }).map(function (l) {
    const parts = l.split('|');
    return { title: (parts[0] || '').trim(), event: (parts[1] || '').trim(), year: (parts[2] || '').trim() };
  });

  const item = {
    id: editId ? Number(editId) : Date.now(),
    title, name, country: country || '', rating: rating || 'N/A',
    style, bio, career: career || '',
    achievements, best_games, image,
  };
...
function editPlayer(id) {
  ...
  document.getElementById('p-achievements').value = achievements.map(function (a) { return a.title + '|' + a.year; }).join('\n');
...
async function addGame() {
  ...
  const white_title = document.getElementById('g-white-title').value || '';
  const black_title = document.getElementById('g-black-title').value || '';
  const white_name  = v('g-white');
  const black_name  = v('g-black');
  const white = white_title ? white_title + ' ' + white_name : white_name;
  const black = black_title ? black_title + ' ' + black_name : black_name;
  const year        = v('g-year');
  const event       = v('g-event');
  const result      = v('g-result');
  const description = v('g-desc');
  const pgn         = v('g-pgn');

  if (!title || !white_name || !black_name) {
    showToast('Title, White, and Black are required.'); return;
  }
  const item = {
    id: editId ? Number(editId) : Date.now(),
    title, white, black, white_title, black_title, white_name, black_name,
    year: year || '', event: event || '', result: result || '',
    description: description || '', pgn: pgn || '',
  };
...
function editGame(id) {
  ...
  document.getElementById('g-white-title').value = g.white_title || '';
  document.getElementById('g-black-title').value = g.black_title || '';
...
async function editPdf(id) {
  ...
  document.getElementById('d-tag').value      = p.tag         || '';
  if (p.file_data) {
    document.getElementById('d-file-data').value              = p.file_data;
    document.getElementById('d-file-name').value              = p.file_name || p.title + '.pdf';
    document.getElementById('d-pdf-preview-name').textContent = p.file_name || p.title + '.pdf';
    document.getElementById('d-pdf-preview-size').textContent = p.size || '';
    document.getElementById('d-pdf-preview').style.display    = 'flex';
    document.getElementById('d-pdf-drop').style.display       = 'none';
  }
  if (p.cover_image) {
    document.getElementById('d-img-data').value = p.cover_image;
    const prev = document.getElementById('d-img-preview');
    const img  = document.getElementById('d-img-preview-img');
    if (img)  img.src = p.cover_image;
    if (prev) prev.style.display = 'flex';
    const drop = document.getElementById('d-img-drop');
    if (drop) drop.style.display = 'none';
  }
...
function cancelEdit(type) {
  const sectionMap = { article: 'articles', player: 'players', game: 'games', pdf: 'pdfs' };
  const prefixMap  = { article: 'a', player: 'p', game: 'g', pdf: 'd' };
  const headings   = { article: 'Add Article', player: 'Add Player Profile', game: 'Add Game of the Century', pdf: 'Add PDF to Library' };
  const labels     = { article: 'Add Article \u2192', player: 'Add Player \u2192', game: 'Add Game \u2192', pdf: 'Add PDF \u2192' };

  const p   = prefixMap[type];
  const sec = sectionMap[type];

  const editId = document.getElementById(p + '-edit-id');
  if (editId) editId.value = '';

  const hEl = document.getElementById(type + '-form-heading');
  if (hEl) hEl.textContent = headings[type];

  const sEl = document.getElementById(p + '-submit-label');
  if (sEl) sEl.textContent = labels[type];

  const cancelBtn = document.getElementById(type + '-cancel-edit');
  if (cancelBtn) cancelBtn.style.display = 'none';

  const form = document.querySelector('#admin-' + sec + ' .admin-form');
  if (form) form.classList.remove('editing');

  const section = document.getElementById('admin-' + sec);
  if (section) {
    ...
    section.querySelectorAll('input[type=checkbox]').forEach(function (el) {
      if (el.id.endsWith('-published')) el.checked = true;
    });
  }
...
// ============================================
// INIT  — no dependency on any SDK or `db` variable
// ============================================
window.addEventListener('DOMContentLoaded', async function () {
  applyStoredTheme();

  // Render board immediately so hero isn't empty while DB loads
  buildHomeBoard();
  ...
  renderAll();
  buildHomeBoard();   // rebuild with player names now loaded
  buildLatestStrip();
  initScrollSpy();
